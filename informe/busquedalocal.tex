...
Para este problema particular consideramos que todo clique es solución, y que en su vencindad se encuentran aquellos cliques que pueden obtenerse aplicando una operación de swap (reemplazar un nodo en el clique por uno que no está), una operación de agregado (de un nodo) o una operación de borrado (de un nodo). En cada iteración del algoritmo, revisamos toda la vecindad y nos quedamos con el clique de mayor frontera. La solución final de la búsqueda local es la mejor de las tres posibilidades de nuestra vecindad. \\
...
El caso fácil es si elegimos los nodos $2$ o $3$, en donde $golosoB$ se queda con la solución $[2,3]$, ya que ni las hojas ni el cuatro generan un clique con mayor frontera que ese par de nodos, mejorando la anterior solución. \\
...
\todo[inline]{Notita para no olvidarse, al explicar los experimentos vamos a tener que decir qué cantidad de iteraciones máximas utilizamos (actualmente está hardcodeado 10)}
...
{\centering
    \includegraphics[width=1\textwidth]{informe/imgs/exp_malo_frontera_greedy_local_2000.pdf} \\
}
$ $\newline